...
比如说，针对一个西瓜质量好坏的预测任务，可以设出以下的表达式：<img src="http://latex.codecogs.com/svg.latex?f(x)=a*x_{1}+b*x_{2}^{2}+c*x_{3}^{3}+d" />
...
其中，距离计算公式可以是如下的表达式：<img src="http://latex.codecogs.com/svg.latex?loss = \sqrt{(f_{1}(x) - y )^2 }" />
...
如图所示就是一个最简单的神经网络结构，这个结构的数学表达式是：<img src="http://latex.codecogs.com/svg.latex?$Y = X1*W1 + X2*W2" />
...
他们最终的计算结果 `Y` 其实说白了就是一个计算公式：![](http://latex.codecogs.com/svg.latex?Y = X1*W1 + X2*W2)这个公式的含义大家应该都明白，**给不同的输入 分配不同的权重 ，从而得到想要的结果。** 
...
**所以，最终的整合式子为：** ![](http://latex.codecogs.com/svg.latex?Y = Y_{11} * W_{3-1} + Y_{12}*W_{3-2} + Y_{13}*W_{3-3})

![](http://latex.codecogs.com/svg.latex?= (X_{1} * W_{1-1} + X_{2}*W_{2-1})*W_{3-1})

![](http://latex.codecogs.com/svg.latex?+ (X_{1} * W_{1-2} + X_{2}*W_{2-2})*W_{3-2} + (X_{1} * W_{1-3} + X_{2}*W_{2-3})*W_{3-3})
...
<img src="http://latex.codecogs.com/svg.latex?J(y,f(x)) = J(w,b) = \frac{1}{m}\sum^{m}_{i=1}|y_{i}-f(x_{i})|" />
...
**公式：**<img src="http://latex.codecogs.com/svg.latex?J(y,f(x)) = J(w,b) = \frac{1}{2m}\sum^{m}_{i=1}(y_{i}-f(x_{i}))^2"/>
